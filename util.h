...
Index<KeyType, KeyComparator> *getInstance(const int type, const uint64_t kt) {
  ...
  else if (type == TYPE_WORMHOLE)
    return new WormholeIndex<KeyType, KeyComparator>(kt);
  else if (type == TYPE_HASHTABLE)
    return new HashTableIndex<KeyType, KeyComparator>(kt);

...
class WormholeIndex : public Index<KeyType, KeyComparator>
{
 public:
  ...
  void UpdateThreadLocal(size_t thread_num) {}
  void AssignGCID(size_t thread_id) {
    kvArray[thread_id] = (struct kv*) malloc(sizeof(KeyType) + 64);
    outArray[thread_id] = (struct kv*) malloc(sizeof(KeyType) + 64);
  ...
  bool insert(KeyType key, uint64_t value, threadinfo *ti) {
    ...
    __buf = __kv->kv;
    __kv->klen = sizeof(key);
    __kv->vlen = 0;
    memcpy(__buf, (char*) &key, sizeof(key)); 
    kv_update_hash(__kv);

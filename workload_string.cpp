...
typedef GenericKey<KEY_BITS> keytype;
typedef GenericComparator<KEY_BITS> keycomp;

extern bool hyperthreading;

using KeyEuqalityChecker = GenericEqualityChecker<KEY_BITS>;
using KeyHashFunc = GenericHasher<KEY_BITS>;
...
inline void load(int wl, 
  ...
  if (kt == EMAIL_KEY && wl == WORKLOAD_A) {
    init_file = "workloads1/email_loada_zipf_int_100M.dat";
    txn_file = "workloads1/email_txnsa_zipf_int_100M.dat";
  } else if (kt == EMAIL_KEY && wl == WORKLOAD_B) {
    init_file = "workloads1/email_loadb_zipf_int_100M.dat";
    txn_file = "workloads1/email_txnsb_zipf_int_100M.dat";
  } else if (kt == EMAIL_KEY && wl == WORKLOAD_C) {
    init_file = "workloads1/email_loadc_zipf_int_100M.dat";
    txn_file = "workloads1/email_txnsc_zipf_int_100M.dat";
  } else if (kt == EMAIL_KEY && wl == WORKLOAD_D) {
    init_file = "workloads1/email_loadd_zipf_int_100M.dat";
    txn_file = "workloads1/email_txnsd_zipf_int_100M.dat";
  } else if (kt == EMAIL_KEY && wl == WORKLOAD_E) {
    init_file = "workloads1/email_loade_zipf_int_100M.dat";
    txn_file = "workloads1/email_txnse_zipf_int_100M.dat";
  } else if (kt == EMAIL_KEY && wl == WORKLOAD_F) {
    init_file = "workloads1/email_loadf_zipf_int_100M.dat";
    txn_file = "workloads1/email_txnsf_zipf_int_100M.dat";
  } else {
...
#else
  if (kt == EMAIL_KEY && wl == WORKLOAD_A) {
    init_file = "workloads1/email_load.dat";
    txn_file = "workloads1/email_a.dat";
  } else if (kt == EMAIL_KEY && wl == WORKLOAD_C) {
    init_file = "workloads1/email_load.dat";
    txn_file = "workloads1/email_c.dat";
  } else if (kt == EMAIL_KEY && wl == WORKLOAD_E) {
    init_file = "workloads1/email_load.dat";
    txn_file = "workloads1/email_e.dat";
  } else {
    fprintf(stderr, "Unknown workload or key type: %d, %d\n", wl, kt);
    exit(1);
  }
...
inline void exec(int wl, 
  ...
  auto func = [idx, &init_keys, num_thread, &values, index_type](uint64_t thread_id, bool) {
    ...
    if(index_type == TYPE_WORMHOLE) {
        unsigned int tid = thread_id;
        ti = reinterpret_cast<threadinfo*>(tid);
    }
...
int main(int argc, char *argv[]) {
  ...
  } else if (strcmp(argv[3], "artrowex") == 0) {
  ...
  } else if (strcmp(argv[3], "hydralist") == 0) {
    index_type = TYPE_HYDRALIST;
  } else if (strcmp(argv[3], "wormhole") == 0) { 
    index_type = TYPE_WORMHOLE;
  } else if (strcmp(argv[3], "hashtable") == 0) { 
    index_type = TYPE_HASHTABLE;
  ...
  int num_thread = atoi(argv[4]); if(num_thread < 1 || num_thread > 500) {

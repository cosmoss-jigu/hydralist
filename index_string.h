...
#include "hydralist_string/include/HydraList.h"
#include "wormhole/wh.h"
#include "hot/src/main.h"
 ...
 public:
  ...
  ArtROWEXIndex(uint64_t kt) {
    if (sizeof(KeyType)==8) {
      ...
      idx = new ART_ROWEX::Tree([](TID tid, Key &key) { key.set(reinterpret_cast<char*>(tid),31); });
      uint8_t m[] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
		     0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
		     0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,
		     0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
...
class HydraListIndex : public Index<KeyType, KeyComparator>
{
 public:
  ...
  bool insert(KeyType key, uint64_t value, threadinfo *ti) {
    if(!idx.insert(key.data, value)) {
            //count++;
            //if(count % 10000 == 0)printf("c = %d\n",count);
    }
    return true;
  }
  ...
  uint64_t scan(KeyType key, int range, threadinfo *ti) {
    ...
    uint64_t size = idx.scan(key.data, range, result);
...
template<typename KeyType, class KeyComparator>
class WormholeIndex : public Index<KeyType, KeyComparator>
{
 public:
  ...
  void UpdateThreadLocal(size_t thread_num) {}
  void AssignGCID(size_t thread_id) {
    kvArray[thread_id] = (struct kv*) malloc(64);
    outArray[thread_id] = (struct kv*) malloc(64);
    refArray[thread_id] = wormhole_ref(wh);
  }
  
  void UnregisterThread(size_t thread_id) {
    struct wormref *ref = refArray[thread_id];
    wormhole_unref(refArray[thread_id]);
    refArray[thread_id] = NULL;
  }

  bool insert(KeyType key, uint64_t value, threadinfo *ti) {
    unsigned long tid = reinterpret_cast<unsigned long>(ti);
    unsigned char *__buf;
    struct kv *__kv;
    __kv = kvArray[tid];
    struct wormref *ref = refArray[tid];

    __buf = __kv->kv;
    __kv->klen = 31;
    __kv->vlen = 0;
    memcpy(__buf, (char*) key.data, 31); 
    kv_update_hash(__kv);
  ...
  uint64_t scan(KeyType key, int range, threadinfo *ti) {
    ...
    int count = 0;
    for (int i = 0; i < range; i++) {
        if (wormhole_iter_next(iter, __kv))
            count++;
        else
            break;
    }
  ...
  WormholeIndex(uint64_t kt) {
    wh = wormhole_create(NULL);
  }


 private:
  struct wormhole *wh;
  struct kv *kvArray[400];
  struct kv *outArray[400];
  struct wormref *refArray[400];

};

template<typename KeyType, class KeyComparator>
class HashTableIndex : public Index<KeyType, KeyComparator>
{
 public:
...
class HOTIndex : public Index<KeyType, KeyComparator>
{
 public:
  ...
  bool insert(KeyType key, uint64_t value, threadinfo *ti) {
    idx.insert(key.data, strlen(key.data), value);
    return true;
  }
  ...
  bool upsert(KeyType key, uint64_t value, threadinfo *ti) {
    ...
    return idx.insert(key.data, strlen(key.data), value);
